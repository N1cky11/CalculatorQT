...
void CalculatorClass::equal() {
   ...
   /* while(!numbers.empty() ) { qDebug() << numbers.top() ; numbers.pop() ;
        qDebug() << tokens.top() ; tokens.pop() ;
        qDebug() << numbers.top() ; numbers.pop() ;
    }

    while(!tokens.empty() ) {
            QChar token=tokens.top() ;
            tokens.pop() ;
            int right = numbers.top();
            numbers.pop();
            int left = numbers.top();
            numbers.pop();
             qDebug() << right<<left<<token ;
            if (token == '+') {
      ...
      qDebug() << numbers.top() ;
      display->setText(QString::number(numbers.top() ) ) ;
}
...
void CalculatorClass:void CalculatorClass::equal() :operandPressed() {
    QPushButton *button=qobject_cast<QPushButton*> (sender() ) ;
    if(!button) return ;

    auto buttonText=button->text() ;
    numbers.push(currentNumber.toInt()) ;
    currentNumber="" ;
